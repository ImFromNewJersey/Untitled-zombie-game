	...
	private void Update () {
		...
		if (Input.GetMouseButton (0) && !outOfAmmo && !isReloading && !isInspecting && !isRunning) 
		{
			...
			if (Time.time - lastFired > 1 / fireRate) 
			{
				...
				//do damage to zombie
				RaycastHit hit;
				if (Physics.Raycast(gunCamera.transform.position, gunCamera.transform.forward, out hit, 100))
				{

					Debug.DrawRay(gunCamera.transform.position, hit.collider.transform.position, Color.red);
					...
					//I think this makes the impact effect
